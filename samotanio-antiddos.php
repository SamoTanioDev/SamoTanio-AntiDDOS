...
*/
	...
	class ProtectionAgainstDDoS{
		...
		function admin_menu()
		{
			add_options_page( 'Anti-DDOS Settings', 'Anti-DDOS', 'install_plugins', $this->option_name, array( $this, 'settings_page' ) );
		}
		...
		{
			$this->activation_error_notice('Can\'t activate the SamoTanio-AntiDDOS plugin because the .htaccess file is not writable. Please make it writable and try again.');
		}

		public function error_notice_mu()
		{
			$this->activation_error_notice('Please activate the SamoTanio-AntiDDOS plugin for the entire network. It can\'t be activated for a particular site within a network.');
		}
...
RewriteRule ^(.*)$ "{$redirect_url}" [R,L,NE]
	...
	//register_activation_hook( WP_PLUGIN_DIR.'/samotanio-anti-ddos/samotanio-anti-ddos.php', array( $this, 'activate' ) );
	register_deactivation_hook( __FILE__, array('SamoTanio-AntiDDOS', 'deactivate' ) );
